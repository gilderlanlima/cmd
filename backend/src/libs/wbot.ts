...
const isValidWaVersion = (version: unknown): version is [number, number, number] => {
  ...
  // Minor fora desse range normalmente indica parse ruim da fonte externa.
  if (minor < 1000 || minor > 9999) {
    return false;
  }

  // Alguns patches oficiais são bem grandes (ex.: 1032954054).
  if (patch < 1) {
    return false;
  }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      (async () => {
        ...
        let versionWA: [number, number, number] | null = null;
        try {
          const fetchedVersion = await Promise.race([
            getVersionByIndexFromUrl(2),
            new Promise<null>((resolve) => setTimeout(() => resolve(null), 8000))
          ]);
          if (isValidWaVersion(fetchedVersion)) {
            versionWA = fetchedVersion;
            console.info("[WBOT.ts] Versao sendo puxada de url:", versionWA);
          } else {
            logger.warn("[WBOT.ts] Versao remota invalida/timeout. Usando versao padrao do Baileys.");
          }
        } catch (error) {
          logger.warn("[WBOT.ts] Falha ao buscar versao remota. Usando versao padrao do Baileys.");
        }
        ...
        if (versionWA) {
          socketConfig.version = versionWA;
        }
  ...
  `})
        ...
        wsocket.ev.on(
          ...
          async ({ connection, lastDisconnect, qr }) => {
            ...
            if (connection === "open") {
              // Sessão conectou com sucesso: zera contador de tentativas de QR.
