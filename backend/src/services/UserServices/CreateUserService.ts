...
import {
  extractLegacyHoursFromWorkSchedule,
  normalizeUserWorkSchedule,
  UserWorkScheduleEntry
} from "../../helpers/UserWorkSchedule";
...
interface Request {
  ...
  startWork?: string;
  endWork?: string;
  workSchedule?: UserWorkScheduleEntry[];
  whatsappId?: number;
  allTicket?: string;
  defaultTheme?: string;
  defaultMenu?: string;
...
const CreateUserService = async ({
  ...
  const normalizedWorkSchedule = normalizeUserWorkSchedule(
    workSchedule,
    startWork || "00:00",
    endWork || "23:59"
  );
  const legacyHours = extractLegacyHoursFromWorkSchedule(
  ...
  const user = await User.create(
    {
      ...
      companyId,
      profile,
      startWork: legacyHours.startWork,
      endWork: legacyHours.endWork,
  ...
  await user.reload({ include: ["queues", "whatsapps", "company"] });
