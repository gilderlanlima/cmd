...
const CreateMessageService = async ({
  ...
  let socketAction: "create" | "update" = "create";

  let storedMessage = await Message.findOne({
    where: {
  ...
  if (storedMessage) {
    socketAction = "update";
    await storedMessage.update({ ...correctedMessageData, companyId });
  } else {
    storedMessage = await Message.create({ ...correctedMessageData, companyId } as any);
  }

  const message = await Message.findByPk(storedMessage.id, {
  ...
  if (message.isPrivate) {
    const privateWid = `PVT${message.id}`;
    if (message.wid !== privateWid) {
      await message.update({ wid: privateWid });
    }
  }
  ...
  if (!messageData?.ticketImported) {
    io.of(String(companyId))
      .emit(`company-${companyId}-appMessage`, {
        ...
        contact: message.ticket?.contact || null
      });
  }
