    ...
    wbot.ev.on("contacts.upsert", async (contacts: BContact[]) => {
      ...
      try {
        for (const contact of contacts) {
          const contactId = (contact as any)?.id || (contact as any)?.jid;
          if (!contactId) {
            continue;
          }

          if (
            isJidBroadcast(contactId) ||
            isJidStatusBroadcast(contactId) ||
          ...
          const contactArray = {
            id: contactId,
            name: contact?.name
              ? cleanStringForJSON(String(contact.name))
              : String(contactId).split("@")[0].split(":")[0]
          };
