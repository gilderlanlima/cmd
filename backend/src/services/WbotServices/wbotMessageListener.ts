...
export const flowbuilderIntegration = async (
  ...
  const configuredFlowId = Number(queueIntegration?.projectName);
  ...
  if (
    ...
    Number.isInteger(configuredFlowId) &&
    configuredFlowId > 0 &&
    !ticket.flowWebhook &&
    messageCount === 1
  ) {
    console.log(
      `[FLOW INTEGRATION] Iniciando fluxo configurado na integração ${queueIntegration.id}: ${configuredFlowId}`
    );
    ...
    try {
      ...
      if (!flow) {
        console.error(
          `[FLOW INTEGRATION] Fluxo ${configuredFlowId} não encontrado para a integração ${queueIntegration.id}`
        );
      } else {
        const nodes: INodes[] = flow.flow["nodes"];
        const connections: IConnections[] = flow.flow["connections"];
        ...
        if (!nodes || !Array.isArray(nodes) || nodes.length === 0) {
          console.error(
            `[FLOW INTEGRATION] Fluxo ${configuredFlowId} não possui nós válidos`
          );
        } else {
          const mountDataContact = {
            number: contact.number,
          ...
          console.log(
            `[FLOW INTEGRATION] Fluxo ${configuredFlowId} executado com sucesso pela integração ${queueIntegration.id}`
          );
      ...
      console.error(
        `[FLOW INTEGRATION] Erro ao executar fluxo ${configuredFlowId} da integração ${queueIntegration.id}:`,
        error
      );
    }
  }
