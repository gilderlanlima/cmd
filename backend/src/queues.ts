    ...
    if (campaigns.length > 0) {
      ...
      const promises = campaigns.map(async campaign => {
        try {
          ...
          logger.info(
            `[VERIFY-CAMPAIGNS] Campanha ${campaign.id} enviada para processamento: Delay=${delay}ms, Horário agendado=${executeAt}, Agora=${now.format('YYYY-MM-DD HH:mm:ss')}`
          );
...
async function verifyAndFinalizeCampaign(campaign) {
  ...
  const verificationPromise = (async () => {
    try {
      ...
      if (!campaignWithContacts?.contactList) {
        logger.warn(`[VERIFY CAMPAIGN] Campanha ${campaignId} não tem lista de contatos`);
        return;
      }

      const { companyId, contacts } = campaignWithContacts.contactList;

      // Contar mensagens entregues usando UPDATE atômico para evitar race conditions
