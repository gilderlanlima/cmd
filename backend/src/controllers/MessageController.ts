...
const getAckFromWhatsAppStatus = (status: unknown): number => {
  const parsed = Number(String(status).replace("PENDING", "2").replace("NaN", "1"));
  return Number.isNaN(parsed) || parsed <= 0 ? 1 : parsed;
};

const getCreatedAtFromWhatsAppMessage = (message: WAMessage): string => {
  const rawTimestamp = Number(message?.messageTimestamp);
  const unixSeconds = Number.isFinite(rawTimestamp) && rawTimestamp > 0
    ? rawTimestamp
  ...
  try {
    if (medias?.length) {
      await Promise.all(
        medias.map(async (media: Express.Multer.File, index) => {
          ...
          if (ticket.channel === "whatsapp") {
            const bodyToPersist = Array.isArray(body) ? body[index] : body;
            ...
            if (isPrivate === "false" && sentMessage?.key?.id) {
              await CreateMessageService({
                ...
                messageData: {
                  ...
                  ticketId: ticket.id,
                  contactId: undefined,
                  body: bodyToPersist || media.originalname || media.filename,
                  fromMe: true,
                  read: true,
                  mediaUrl: media.filename,
                  mediaType: getMediaTypeFromMimeType(media.mimetype),
                  quotedMsgId: quotedMsg?.id,
                  ack: getAckFromWhatsAppStatus(sentMessage?.status),
                  ...
                  isPrivate: false,
                  isForwarded: false,
                  createdAt: getCreatedAtFromWhatsAppMessage(sentMessage)
                }
              });
            }
          }
      ...
      if (ticket.channel === "whatsapp" && isPrivate === "false") {
        const sentMessage = await SendWhatsAppMessage({ body, ticket, quotedMsg, vCard });
