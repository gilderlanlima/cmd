  ...
  const handleimportContact = async () => {
    try {
      ...
      if (imported > 0 || updated > 0) {
        toast.success(
          `Importação concluída: ${imported} novo(s) e ${updated} atualizado(s).`
        );
      } else if (totalCandidates === 0) {
        toast.info("Nenhum contato disponível para importação no momento.");
      } else {
        toast.info("Nenhum novo contato encontrado para importar no momento.");
      }
