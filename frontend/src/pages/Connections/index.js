...
const Connections = () => {
  ...
  useEffect(() => {
    const hasOpeningSession = whatsApps?.some(
      (wpp) => wpp?.channel === "whatsapp" && wpp?.status === "OPENING"
    );
  ...
  const renderActionButtons = (whatsApp) => {
    return (
      <>
        {whatsApp.channel === "whatsapp" && whatsApp.status !== "qrcode" && !!whatsApp.qrcode && (
          <Can
