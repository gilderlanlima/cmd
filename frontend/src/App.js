...
const BRAND_PRIMARY_LIGHT = "#2f6fed";
const BRAND_PRIMARY_DARK = "#1d4ed8";
const BRAND_ACCENT = "#37d3f4";
const BRAND_BACKGROUND_LIGHT = "#f3f6fb";
const BRAND_BACKGROUND_DARK = "#111b2a";
const BRAND_TOPBAR_DARK = "#1c2a3a";

const App = () => {
  const [locale, setLocale] = useState();
  const appColorLocalStorage = BRAND_PRIMARY_LIGHT;
      ...
      createTheme(
        {
          ...
          palette: {
            ...
            secondary: { main: BRAND_ACCENT, contrastText: "#0f1724" },
            textPrimary:
              mode === "light" ? primaryColorLight : primaryColorDark,
            borderPrimary:
              mode === "light" ? primaryColorLight : primaryColorDark,
            dark: { main: mode === "light" ? "#1f2d3d" : "#F3F3F3" },
            light: { main: mode === "light" ? "#F3F3F3" : "#333333" },
            fontColor: mode === "light" ? primaryColorLight : primaryColorDark,
            tabHeaderBackground: mode === "light" ? "#e8eff8" : "#1f2b3b",
            optionsBackground: mode === "light" ? "#f8fbff" : "#152235",
            fancyBackground: mode === "light" ? "#f8fbff" : "#152235",
            total: mode === "light" ? "#fff" : "#222",
            messageIcons: mode === "light" ? "#5f6f83" : "#F3F3F3",
            inputBackground: mode === "light" ? "#FFFFFF" : "#162133",
            background: {
              default:
                mode === "light" ? BRAND_BACKGROUND_LIGHT : BRAND_BACKGROUND_DARK,
              paper: mode === "light" ? "#ffffff" : "#172335",
            },
            barraSuperior:
              mode === "light" ? primaryColorLight : BRAND_TOPBAR_DARK, // Usa cor do tema
          },
  ...
  useEffect(() => {
    setPrimaryColorLight(BRAND_PRIMARY_LIGHT);
    setPrimaryColorDark(BRAND_PRIMARY_DARK);
    window.localStorage.setItem("primaryColorLight", BRAND_PRIMARY_LIGHT);
    window.localStorage.setItem("primaryColorDark", BRAND_PRIMARY_DARK);
  ...
  useEffect(() => {
    async function fetchVersionData() {
      try {
        const response = await openApi.get("/version");
        const { data } = response;
