...
const onCompanyAppMessageNotificationsPopover = (data) => {
	const ticketUserId = normalizeToNumber(data?.ticket?.userId);
	const currentUserId = normalizeToNumber(user?.id);
	const ticketQueueId = normalizeToNumber(data?.ticket?.queueId);

    if (
        data.action === "create" && !data.message.fromMe &&
        !data.message.read &&
        (ticketUserId === currentUserId || !ticketUserId) &&
        (user?.queues?.some(queue => normalizeToNumber(queue.id) === ticketQueueId) ||
