...
const QueueModal = ({ open, onClose, queueId, onEdit, onSaved }) => {
  ...
  const handleSaveQueue = async (values) => {
    try {
      ...
      if (queueId) {
        response = await api.put(`/queue/${queueId}`, { ...values, schedules });
      } else {
        response = await api.post("/queue", { ...values, schedules });
      }
