...
const NewTicketModal = ({ modalOpen, onClose, initialContact }) => {
  ...
  const availableWhatsappIds = useMemo(
    () => user.whatsapps?.map((item) => item.id) || [],
    [user.whatsapps]
  );
    ...
    const delayDebounceFn = setTimeout(() => {
      const fetchContacts = async () => {
        api
          ...
          .then(({ data }) => {
            const filteredWhatsapps = availableWhatsappIds.length
              ? data.filter((whatsapp) =>
                  availableWhatsappIds.includes(whatsapp.id)
                )
              : data;
  ...
  }, [selectedContact, channelFilter, availableWhatsappIds, companyId, user.queues, whatsappId])
