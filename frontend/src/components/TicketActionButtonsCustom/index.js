...
import { emitTicketSync } from "../../utils/ticketSync";
...
const TicketActionButtonsCustom = ({
  ...
  const handleCloseTicketWithoutFarewellMsg = async () => {
    ...
    try {
      ...
      emitTicketSync({ action: "update", ticket: updatedTicket });
  ...
  const handleDeleteTicket = async () => {
    try {
      ...
      emitTicketSync({ action: "delete", ticketId: ticket.id });
      history.push("/tickets");
    } catch (err) {
      toastError(err);
    }
  ...
  const handleAcepptTicket = async (id) => {
    ...
    try {
      ...
      emitTicketSync({ action: "update", ticket: otherTicket.data });
