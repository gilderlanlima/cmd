...
const TicketsListCustom = (props) => {
    ...
    useEffect(() => {
        ...
        const selectedQueueIdsNormalized = selectedQueueIds
            .map(normalizeToNumber)
            .filter((id) => id !== null);
        const hasQueueFilter = selectedQueueIdsNormalized.length > 0;

        const shouldUpdateTicket = ticket => {
            const ticketUserId = normalizeToNumber(ticket?.userId);
            const currentUserId = normalizeToNumber(user?.id);
            const ticketQueueId = normalizeToNumber(ticket?.queueId);
            const queueMatches = hasQueueFilter
                ? ((!ticketQueueId && showTicketWithoutQueue) || selectedQueueIdsNormalized.indexOf(ticketQueueId) > -1)
                : true;

            return (!ticketUserId || ticketUserId === currentUserId || showAll) &&
                queueMatches
        ...
        const onCompanyTicketTicketsList = (data) => {
            ...
            if (data.action === "update" && data.ticket) {
                // A aba de busca funciona sob demanda; evita alterar resultados
                // dinamicamente e mantém o comportamento atual.
                if (status === "search") {
                    return;
                }
                ...
                // Remove da lista atual quando o ticket muda de status/fila/usuário.
                dispatch({
                    type: "DELETE_TICKET",
        ...
        const onLocalTicketSync = (event) => {
            onCompanyTicketTicketsList(event?.detail);
        };
        ...
        window.addEventListener("ticket:sync", onLocalTicketSync);
    ...
    useEffect(() => {
            ...
            try {
                const { data } = await api.get("/tickets", {
                    params: {
                        ...
                        showAll,
                        queueIds: JSON.stringify(selectedQueueIds),
                        whatsapps: JSON.stringify(whatsappIds),
                        statusFilter: JSON.stringify(statusFilter),
                        sortTickets,
                        searchOnMessages: searchOnMessages ? "true" : "false"
                    },
        ...
        fetchLatestTickets();
        const interval = setInterval(fetchLatestTickets, 3000);
