...
const reducer = (state, action) => {
  if (action.type === "LOAD_MESSAGES") {
    ...
    const messagesMap = new Map(state.map((message) => [message.id, message]));

    messages.forEach((message) => {
      messagesMap.set(message.id, message);
    });

    return sortMessagesByCreatedAt(Array.from(messagesMap.values()));
  }
  ...
  if (action.type === "ADD_MESSAGE") {
    ...
    if (messageIndex !== -1) {
      nextState[messageIndex] = newMessage;
    } else {
      nextState.push(newMessage);
    }

    return sortMessagesByCreatedAt(nextState);
  }
  ...
  if (action.type === "UPDATE_MESSAGE") {
    ...
    if (messageIndex !== -1) {
      nextState[messageIndex] = messageToUpdate;
    } else {
      nextState.push(messageToUpdate);
    }
...
const MessagesList = ({
  ...
  const syncMessagesNow = useCallback(async () => {
    if (ticketId === "undefined" || isNil(ticketId)) {
      return;
    }
    ...
    try {
      ...
      if (Array.isArray(data?.messages)) {
        if (data?.ticket?.id) {
          currentTicketNumericId.current = String(data.ticket.id);
        }

        const latestMessageId = data.messages[data.messages.length - 1]?.id || null;
        const shouldScrollBottom = latestMessageId && lastKnownMessageId.current !== latestMessageId;
      ...
      // fallback silencioso para nao gerar ruido visual
    }
  ...
  useEffect(() => {
    lastKnownMessageId.current = messagesList?.[messagesList.length - 1]?.id || null;
  }, [messagesList]);
  ...
  useEffect(() => {
    ...
    const onAppMessageMessagesList = (data) => {
      const routeTicketRef = String(ticketId || "");
      const messageTicketUuid = data?.message?.ticket?.uuid;
      const payloadTicketUuid = data?.ticket?.uuid;
      const messageTicketId = data?.message?.ticketId || data?.message?.ticket?.id;
      const payloadTicketId = data?.ticket?.id;
      const ticketNumericRef = String(currentTicketNumericId.current || "");
      const isCurrentTicket =
        payloadTicketUuid === routeTicketRef ||
        messageTicketUuid === routeTicketRef ||
        String(messageTicketId || "") === routeTicketRef ||
        String(payloadTicketId || "") === routeTicketRef ||
        String(messageTicketId || "") === ticketNumericRef ||
        String(payloadTicketId || "") === ticketNumericRef;
      ...
      if (data.action == "delete" && isCurrentTicket) {
  ...
  useEffect(() => {
    ...
    syncMessagesNow();
    const interval = setInterval(syncMessagesNow, 2500);

    return () => {
      clearInterval(interval);
    };
  }, [ticketId, syncMessagesNow]);

  useEffect(() => {
    const onMessageSync = (event) => {
      const syncTicketId = String(event?.detail?.ticketId || "");
      if (syncTicketId && syncTicketId !== String(ticketId)) {
        return;
      }
      syncMessagesNow();
    };

    window.addEventListener("message:sync", onMessageSync);

    return () => {
      window.removeEventListener("message:sync", onMessageSync);
    };
  }, [ticketId, syncMessagesNow]);
